...
NEW_WEBHOOK="http://lobe-chat:3210"

if grep -qE "${PROF_REDIRECT_MATCH}|localhost:3210" "$INIT_DATA"; then
  cp "$INIT_DATA" "$INIT_DATA.bak.$(date +%s)"
  # Replace Casdoor prefix first, then LobeChat. gsub args are taken as
  # literals here (no regex metacharacters in the host:port prefixes except
  # '.', which still matches the literal dots).
  jq --arg ol "$OLD_LOBE" --arg nl "$NEW_LOBE" \
     --arg oc "$OLD_CASDOOR" --arg nc "$NEW_CASDOOR" \
     --arg ow "$OLD_WEBHOOK" --arg nw "$NEW_WEBHOOK" '
    walk(
      if type == "string"
      then (gsub($oc; $nc) | gsub($ol; $nl) | gsub($ow; $nw))
      else . end
    )
  ' "$INIT_DATA" > "$INIT_DATA.tmp"
  mv "$INIT_DATA.tmp" "$INIT_DATA"
  log "init_data.json patched: LobeChat -> $NEW_LOBE , Casdoor -> $NEW_CASDOOR , webhook -> $NEW_WEBHOOK"
else
  log "no professor/localhost URLs found in init_data.json — assuming already patched"
fi
